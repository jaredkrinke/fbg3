...
Game.prototype.pieceTryRotate = function (offset) {
    let rotationIndex = (this.pieceRotationIndex + offset) % Constants.pieces[this.pieceIndex].length;
...
Game.prototype.gameAdvanceFrame = function (upPressed, downPressed, leftPressed, rightPressed, cwPressed, ccwPressed) {
    let inputRead = false;
    if (this.result === null) {
        this.frame++;
...
Replay.prototype.advanceFrame = function () {
    ...
    const upPressed = ((input & 0x1) !== 0);
    const downPressed = ((input & 0x2) !== 0);
    const leftPressed = ((input & 0x4) !== 0);
    const rightPressed = ((input & 0x8) !== 0);
    const cwPressed = ((input & 0x10) !== 0);
    const ccwPressed = ((input & 0x20) !== 0);
    const inputRead = this.game.gameAdvanceFrame(upPressed, downPressed, leftPressed, rightPressed, cwPressed, ccwPressed);

    // Back up if input wasn't actually read (TODO: this is arguably a bug in the replay protocol...)
    if (!inputRead) {
        this.index--;
    }
